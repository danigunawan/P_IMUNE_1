...
if (!empty($credentials['email'])) 
{
	...
	if (is_array($row_item))
	{
		if (array_key_exists('id', $row_item))
		{
			...
			$mail->AddAddress($email, "layanan akun". $app_brand);
			$mail->Send();

			$success = true;
			$message = 'Sandi untuk situs telah berubah benar.';
		}
	}
	else
	{
		$message = 'Akun dengan alamat email yang diberikan belum ditemukan.';
	}
}
