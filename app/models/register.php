...
if (!empty($credentials['login']) && !empty($credentials['email']) && !empty($credentials['password'])) 
{
	...
	if (is_array($row_item))
	{
		...
		{
			$role = 3;
			$active = 0; //ini untuk status aktif atau tidak saat register pertama kali jika 1 = maka aktif langsung jika 0 = maka harus diaktifkan oleh admin
			...
			$message = 'Anda berhasil terdaftar ke situs.';
			$success = true;
		}
		else 
		{
			$message = 'Ditentukan nama atau e-mail yang sudah ada.';
		}
	}
}
